...
//	}
	...
	public void test01GameWaterfallRank() {
		...
		solo.sleep(2000);
		boolean isNoGameTip = solo.searchText("已经没有更多游戏");
		assertEquals("没有更多游戏的提示", true, isNoGameTip);
...
//	}
	...
	public void test02GameListRank() {
		...
		String[] GameName = new String[gamecount];
//		try {
			for (int i = 2; i < gameCount-1; i++) {
				...
				GameName[i-2] = ((WyxGameCenter) GameListView2.getItemAtPosition(i)).getAppname();
...
//				Log.e("游戏中心列表", "  "+i+":" + GameName[i].getText());
			...
			assertEquals("第1个游戏不是‘超三国’？", "超三国", GameName[0]);
			assertEquals("第2个游戏不是‘微城市’？", "微城市", GameName[1]);
			assertEquals("第3个游戏不是‘你画我猜移动版’？", "你画我猜移动版", GameName[2]);
			assertEquals("第4个游戏不是‘爱画爱猜’？", "爱画爱猜", GameName[3]);
